...
public class TactilePlayer extends Player {
    ...
    public TactilePlayer(int brailleCellNumber, int buttonNumber) {
        ...
        try {
            ...
            ON = gpio.getGPIO().provisionDigitalOutputPin(RaspiPin.GPIO_00, "ON", PinState.LOW);
            DOUT = gpio.getGPIO().provisionDigitalOutputPin(RaspiPin.GPIO_01, "DOUT", PinState.LOW);
            STROBE = gpio.getGPIO().provisionDigitalOutputPin(RaspiPin.GPIO_02, "STROBE", PinState.LOW);
            CLOCK = gpio.getGPIO().provisionDigitalOutputPin(RaspiPin.GPIO_03, "CLOCK", PinState.LOW);
            ON.high();
            Thread.sleep(2000);
            ON.low();
            ...
            for (int i = 0; i < buttonNumber; i++) {
                ...
                GpioPinDigitalInput button = gpio.getGPIO().provisionDigitalInputPin(RaspiPin.getPinByAddress(i+4), PinPullResistance.PULL_DOWN);
